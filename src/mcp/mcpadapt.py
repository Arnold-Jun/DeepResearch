...
class MCPAdapt():
    def __init__(
        ...
        # Extract mcpServers from the config for fastmcp.Client
        server_params = config.get('mcpServers', {})
        if not server_params:
            raise ValueError("MCPAdapt config must contain 'mcpServers' key.")
    ...
    async def tools(self):
        ...
        async with self.client as client:
            ...
            # Adapt tools while client is still available
