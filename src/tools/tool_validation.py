...
class MethodChecker(ast.NodeVisitor):
    ...
    检查方法是否：
    - 仅使用已定义的名称
    - 不包含本地导入（例如 numpy 可以，但 local_script 不行）
    """
    ...
    def visit_With(self, node):
        """跟踪 'with' 语句中的别名（'with X as y' 中的 'y'）"""
    ...
    def visit_ExceptHandler(self, node):
        """跟踪异常别名（'except Exception as e' 中的 'e'）"""
    ...
    def visit_Name(self, node):
        if isinstance(node.ctx, ast.Load):
            if not (
                ...
                self.errors.append(f"名称 '{node.id}' 未定义。")
    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Name):
            if not (
                ...
                self.errors.append(f"名称 '{node.func.id}' 未定义。")
        self.generic_visit(node)
...
def validate_tool_attributes(cls, check_imports: bool = True) -> None:
    """
    验证 Tool 类是否遵循正确的模式：
    0. __init__ 的任何参数都应该有默认值。
    在初始化时选择的参数不可追踪，因此我们无法为它们重建源代码，因此任何重要参数都应定义为类属性。
    1. 关于类：
        - 类属性应该只是字符串或字典
        - 类属性不能是复杂属性
    2. 关于所有类方法：
        ...
        def visit_Assign(self, node):
            ...
            if getattr(node.targets[0], "id", "") == "name":
                if not isinstance(node.value, ast.Constant):
                    self.invalid_attributes.append(f"类属性 'name' 必须是常量，发现 '{node.value}'")
                elif not isinstance(node.value.value, str):
                    self.invalid_attributes.append(
                        f"类属性 'name' 必须是字符串，发现 '{node.value.value}'"
                    )
                elif not is_valid_name(node.value.value):
                    self.invalid_attributes.append(
                        f"类属性 'name' 必须是有效的 Python 标识符且不能是保留关键字，发现 '{node.value.value}'"
                    )
    ...
    if errors:
        raise ValueError(f"工具验证失败 {cls.__name__}：\n" + "\n".join(errors))
    return
