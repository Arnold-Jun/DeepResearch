...
class BingSearchEngine(WebSearchEngine):
    ...
    def _search_sync(self, query: str, num_results: int = 10) -> List[SearchItem]:
        """
        Synchronous Bing research implementation to retrieve research results.

        Args:
            query (str): The research query to submit to Bing.
            num_results (int, optional): Maximum number of results to return. Defaults to 10.

        Returns:
            List[SearchItem]: A list of research items with title, URL, and description.
        """
        if not query:
            return []
    ...
    def _parse_html(
        ...
        """
        Parse Bing research result HTML to extract research results and the next page URL.
